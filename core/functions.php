...
function checkIsNotLogin()
{
	if (!isset($_SESSION['login'])) {
		header("Location:http://localhost/perpustakaancuyyy/login");
	}
}

function urlTo($to)
{
	return 'http://localhost/perpustakaancuyyy'.$to;
}

function redirectTo($icon, $pesan, $tujuan)
{
	setcookie('alert', serialize([$icon, $pesan]), time() + 1, '/');
	header("Location:http://localhost/perpustakaancuyyy".$tujuan);
}
